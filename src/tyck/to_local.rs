...
fn eval(env: List<Value>, tm: Expr<Lvl>) -> Value {
    ...
    match tm {
        Expr::Ref(idx) => env
            .iter()
            .nth(idx.0)
            .expect(&format!("unexpected error: get ref {} failed", idx.0))
            .clone(),
        ...
        Expr::Dt(is_pi, Param(name, term), term1) => {
            if is_pi {
                Value::Pi(
                    name,
                    Box::new(eval(env.clone(), *term)),
                    Closure(env, *term1),
                )
            } else {
                Value::Sig(
                    Box::new(eval(env.clone(), *term)),
                    Box::new(eval(env, *term1)),
                )
            }
        }
        Expr::Pair(a, b) => Value::Tuple(Box::new(eval(env.clone(), *a)), Box::new(eval(env, *b))),
...
fn quote(level: usize, value: Value) -> Expr<usize> {
    match value {
        ...
        Value::App(vf, va) => Expr::App(Box::new(quote(level, *vf)), Box::new(quote(level, *va))),
        ...
        Value::Sig(a, b) => todo!(),
        Value::U => Expr::Univ,
        Value::Two(a, b) => Expr::Two(Box::new(quote(level, *a)), Box::new(quote(level, *b))),
...
pub fn infer(cxt: &Cxt, t: &Decl<String>) -> Result<(Decl<Lvl>, Value, Cxt), (String, usize)> {
    match t {
        Decl::Def {
            ...
            let param: Vec<Param<usize>> = tele
                .iter()
                .map(|x| {
                    //println!(">>>> {}", x.0.0);
            ...
            let typ = tele.iter().rev().fold(result.clone(), |a, b| {
                Expr::Dt(true, b.clone(), Box::new(a))
            });
            let bod = tele
                .iter()
                .rev()
                .fold(body.clone(), |a, b| Expr::Lam(b.0.clone(), Box::new(a)));
            {
...
fn infer_expr(cxt: &Cxt, t: &Expr<String>) -> Result<(Expr<Lvl>, Value), (String, usize)> {
    match t {
        ...
        Expr::Dt(is_pi, Param(name, expr), b) => {
            if *is_pi {
                ...
                //println!("{:?}  {:?}", b, b_tm);
                Ok((
                    Expr::Dt(*is_pi, Param(name.clone(), Box::new(a_tm)), Box::new(b_tm)),
                    Value::U,
                ))
            } else {
                //TODO:
                ...
                Ok((
                    Expr::Dt(
                        *is_pi,
                        Param(name.clone(), Box::new(expr_tm)),
                        Box::new(expr1_tm),
                    ),
                    Value::U,
                ))
        ...
        Expr::Two(t, u) => {
            ...
            match tty {
                ...
                _ => report(
                    cxt,
                    &format!("Expected a function type but get {:?}:{:?}", t, tty),
                ),
            }
        }
        Expr::Fst(expr) => {
            ...
            match (expr_tm, expr_ty) {
                (e, Value::Sig(fst, _)) => Ok((Expr::Fst(Box::new(e)), *fst)),
                (_, expr_ty) => report(
                    cxt,
                    &format!(
                        "Expected a pair type for fst, but get {:?}. infer from {:?}",
                        expr_ty, expr
                    ),
                ),
            }
        ...
        Expr::Snd(expr) => {
            let (expr_tm, expr_ty) = infer_expr(cxt, expr)?;
            match (expr_tm, expr_ty) {
                (e, Value::Sig(_, snd)) => Ok((Expr::Snd(Box::new(e)), *snd)),
                (_, expr_ty) => report(
                    cxt,
                    &format!(
                        "Expected a pair type for snd, but get {:?}. infer from {:?}",
                        expr_ty, expr
                    ),
                ),
            }
    ...
    match (t, u) {
        ...
        (Value::Pi(_, a, b), Value::Pi(_, a_, b_)) => {
            conv(l, a, a_)
                && conv(
                    l + 1,
                    &b.clone().apply(Value::Lvl(l)),
                    &b_.clone().apply(Value::Lvl(l)),
                )
        }
        (Value::Lam(_, t), Value::Lam(_, t_)) => conv(
            l + 1,
            &t.clone().apply(Value::Lvl(l)),
            &t_.clone().apply(Value::Lvl(l)),
        ),
        (Value::Lam(_, t), u) => conv(
            l + 1,
            &t.clone().apply(Value::Lvl(l)),
            &Value::App(Box::new(u.clone()), Box::new(Value::Lvl(l))),
        ),
        (u, Value::Lam(_, t)) => conv(
            l + 1,
            &Value::App(Box::new(u.clone()), Box::new(Value::Lvl(l))),
            &t.clone().apply(Value::Lvl(l)),
        ),
